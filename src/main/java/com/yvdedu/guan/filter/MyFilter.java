package com.yvdedu.guan.filter;
...
public class MyFilter extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
			HttpServletResponse response, Object handler) throws Exception {//postHandle、afterCompletion这两个是处理后拦截
		System.out.println(request.getServletPath());//打印出请求路径
		if("/user/login".equals(request.getServletPath())){
			System.out.println(request.getParameter("name"));
			System.out.println(request.getParameter("password"));
			return true;
		}
		return true;//false就是不放行  true就是放行   这个拦截器里面的其他操作都一样 可以跳转等等
	}
